...
const defaultPackageConf = {
  ...
  'version-string': {
    ...
    LegalCopyright: `Copyright © ${(new Date()).getFullYear()} ${packageJSON.author.name}, All rights reserved.`,
    FileDescription: `${packageJSON.productName} by ${packageJSON.author.name}`,
...
gulp.task('package:darwin', ['clean-dist-darwin', 'build'], (done) => {
  packager(_.extend({}, defaultPackageConf, { platform: 'darwin' }), done); // eslint-disable-line
});
...
gulp.task('build', ['transpile', 'images', 'less', 'fonts', 'html']);
